...
class WSFrameBuilderTest(unittest.TestCase):
    ...
    def test_frame_too_large(self):
        f = Frame(opcode=OPCODE_TEXT, body=enc(''), fin=1)
        # fake huge length
...
class WSFrameParserTest(unittest.TestCase):
    ...
    def test_rsv1_bits_set(self):
        f = Frame()
        self.assertRaises(ProtocolException, f.parser.send, enc('\x40'))

    def test_rsv2_bits_set(self):
        f = Frame()
        self.assertRaises(ProtocolException, f.parser.send, enc('\x20'))

    def test_rsv3_bits_set(self):
        f = Frame()
        self.assertRaises(ProtocolException, f.parser.send, enc('\x10'))

    def test_invalid_opcode(self):
        for opcode in range(3, 9):
            f = Frame()
            self.assertRaises(ProtocolException, f.parser.send, enc(chr(opcode)))

        f = Frame()
        self.assertRaises(ProtocolException, f.parser.send, enc(chr(10)))

    def test_fragmented_control_frame_is_invalid(self):
        f = Frame()
        self.assertRaises(ProtocolException, f.parser.send, enc('0x9'))
